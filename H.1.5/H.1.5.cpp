...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		case ID_FILE_CLASS1:
			++cnt1;
			str.clear();
			str << "Class 1 - Window " << cnt1;
			hWndtmp = CreateWindow(L"Class 1", str.str().c_str(), WS_OVERLAPPEDWINDOW,
		...
		case ID_FILE_CLASS2:
			++cnt2;
			str.clear();
			str << "Class 2 - Window " << cnt2;
			hWndtmp = CreateWindow(L"Class 2", str.str().c_str(), WS_OVERLAPPEDWINDOW,
				CW_USEDEFAULT, 0, CW_USEDEFAULT, 0, NULL, NULL, hInst, NULL);
			ShowWindow(hWndtmp, 1);
			break;
		case ID_FILE_CLASS3:
			++cnt3;
			str.clear();
			str << "Class 3 - Window " << cnt3;
			hWndtmp = CreateWindow(L"Class 3", str.str().c_str(), WS_OVERLAPPEDWINDOW,
				CW_USEDEFAULT, 0, CW_USEDEFAULT, 0, NULL, NULL, hInst, NULL);
			ShowWindow(hWndtmp, 1);
			break;
		default:
...
LRESULT CALLBACK WndProc3(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_LBUTTONDOWN:
	{
		...
		int y = GET_Y_LPARAM(lParam);
		hdc = GetDC(hWnd);
		SelectObject(hdc, CreateSolidBrush(RGB(255, 128, 64)));
